...
var deviceId;
var userData = {};
var qualities = {0:'adaptive',1:'low',2:'mid',3:'high',4:'ultra'};
...
function initialize(details){
  chrome.storage.local.get(['userData'],function(items){
	...
	userData['volumeScroll'] = (items.userData!==undefined&&items.userData['volumeScroll']!==undefined)?items.userData['volumeScroll']:'true';
	userData['deviceType'] = (items.userData!==undefined&&items.userData['deviceType']!==undefined)?items.userData['deviceType']:0;
...
function getMediaInfo(mediaId,quality,sendResponse,local){
  ...
  $.ajax('https://api.crunchyroll.com/info.0.json',{data:option,success:function(data){
   if(data.error){
	 ...
	 var tempQuality = 0;
	 for(var x=0;x<data.data.stream_data.streams.length;x++){
	   if(data.data.stream_data.streams[x].quality == qualities[quality]){
	     tempQuality = x;
		 break;
	   }
	 }
     sendResponse({url:data.data.stream_data.streams[tempQuality].url,playhead:data.data.playhead,availQuality:data.data.stream_data.streams});
  }}});  
}
...
chrome.runtime.onMessage.addListener(function(message,sender,sendResponse){
  ...
  if(message.text === "getMediaUrl"){
    ...
    sendResponse({'bufferLength':userData['bufferLength'],'skipOp':userData['skipOp'],'skipEd':userData['skipEd'],'locale':userData['locale'],'resume':userData['resume'],'start':userData['start'],'isPremium':userData['premium'],'autoplay':userData['autoplay'],width:userData['width'],height:userData['height'],volumeScroll:userData['volumeScroll'],deviceType:userData['deviceType']});
	...
	chrome.storage.local.set({'userData':userData});
  }else if(message.text === "updateVolumeScroll"){
    userData['volumeScroll'] = message.volumeScroll;
	chrome.storage.local.set({'userData':userData});
  }else if(message.text === "updateDeviceType"){
    var deviceTypes = ['iphone','android','windowsDesktop','windowsPhone','windowsTablet'];
	userData['deviceType'] = deviceTypes.indexOf(message.deviceType);
	chrome.storage.local.set({'userData':userData});
	start(null,true,null);
  }
});