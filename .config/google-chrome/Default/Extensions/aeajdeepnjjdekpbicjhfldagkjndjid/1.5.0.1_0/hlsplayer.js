...
$(function(){  
  chrome.runtime.sendMessage({text:"getOptions"},function(reply){
    ...
    width = reply.width;
	height = reply.height;
	volumeScroll = reply.volumeScroll === "true"?true:false;
...
function getMediaInfo(mediaId,quality){
  chrome.runtime.sendMessage({text:"getMediaUrl",mediaId:mediaId,quality:quality},function(reply){
	...
	var tempH = 0;
	for(var x=1;x<reply.availQuality.length;x++){
	  if(parseInt(reply.availQuality[x].bitrate)==tempH){
	    $('.showmedia-btns a.button:nth-child('+x+')').hide();
		if(!changedButton){
		  $('.showmedia-btns a.button.selected').removeClass('selected dark-button default-button');
		  $('.showmedia-btns a.button:nth-child('+(x-1)+')').toggleClass('selected dark-button default-button');
		  changedButton = true;
		}
	  }
	  tempH = parseInt(reply.availQuality[x].bitrate);
	}
	...
	if(width != 960){
	targetOffset = (width-960)/2
	targetOffset.toPrecision(1);
	$('#video').css('right',targetOffset.toString()+'px');
	}
    ...
    player.hotkeys({alwaysCaptureHotkeys:true,enableVolumeScroll:volumeScroll});
	ccount = 0;
